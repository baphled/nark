...
== Configuring that Tracker

  Rack::Tracker.config do |config|
    config.plugins_paths << 'some-plugin-path'
    config.plugins = [:request_times, :requests]
  end
...
eventually end up.
  ...
  Rack::Tracker::DefinePlugin.new :request_times do
    Rack::Tracker.plugin_variables :last_request_time => nil, :request_times => []

    Rack::Tracker.plugin_method :last_request_time do
      Rack::Tracker.last_request_time
    end

    Rack::Tracker.plugin_method :last_request_time= do |time|
      Rack::Tracker.last_request_time = time
    end

    Rack::Tracker.plugin_method :request_times do
      Rack::Tracker.request_times
    end
